...
class PurgeUnusedAction(PySide.QtGui.QAction):

    def __init__(self):
         PySide.QtGui.QAction.__init__(self, "Purge Unused Clips", None)
         self.triggered.connect(self.PurgeUnused)
         hiero.core.events.registerInterest("kShowContextMenu/kBin", self.eventHandler)
         self.setIcon(PySide.QtGui.QIcon('icons:TagDelete.png'))
    ...
    def PurgeUnused(self) :
        ...
        if len(SEQS)==0:
            ...
            msgBox.setInformativeText("You have no Sequences in this Project. Do you want to remove all Clips (%i) from Project: %s?" % (len(CLIPSTOREMOVE),proj.name()));
        ...
        msgBox = QMessageBox()
        msgBox.setText("Purge Unused Clips");
        msgBox.setInformativeText("Remove %i unused Clips from Project %s?" % (len(CLIPSTOREMOVE),proj.name()));
        msgBox.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel);
        msgBox.setDefaultButton(QMessageBox.Ok);
        ret = msgBox.exec_()
